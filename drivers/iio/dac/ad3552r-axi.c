...
#define AD3552R_MAX_SAMPLE_RATE_HZ	(33 * MEGA)
...
static int ad3552r_axi_qspi_read(struct iio_backend *back,
				 u32 reg, u32 *val, size_t xfer_size)
{
	return iio_backend_bus_reg_read(back, reg, val, xfer_size);
}

static int ad3552r_axi_qspi_write(struct iio_backend *back,
				  u32 reg, u32 val, size_t xfer_size)
{
	return iio_backend_bus_reg_write(back, reg, val, xfer_size);
}
					 ...
					 size_t xfer_size)
{
	...
	err = ad3552r_axi_qspi_read(back, reg, &rval, xfer_size);
	if (err)
		return err;
	...
	return ad3552r_axi_qspi_write(back, reg, rval, xfer_size);
}
				 ...
				 int val, int val2, long mask)
{
	...
	switch (mask) {
	case IIO_CHAN_INFO_RAW:
		return ad3552r_axi_qspi_write(st->back,
					      AD3552R_REG_ADDR_CH_DAC_16B(ch),
					      (u32)val,
					      SZ_16);
	default:
...
static int ad3552r_axi_buffer_postenable(struct iio_dev *indio_dev)
{
	...
	fmt.type = IIO_BACKEND_DATA_USIGN;
...
static int ad3552r_axi_buffer_predisable(struct iio_dev *indio_dev)
{
	...
	return iio_backend_buffer_disable(st->back);
}
...
static int ad3552r_axi_reset(struct ad3552r_axi_state *st)
{
	...
	if (st->reset_gpio) {
		...
		err = axi3552r_qspi_update_reg_bits(st->back,
					...
					AD3552R_MASK_SOFTWARE_RESET, SZ_8);
		if (err)
			return err;
	}
	fsleep(100000);
...
static int ad3552r_axi_setup(struct ad3552r_axi_state *st)
{
	...
	err = ad3552r_axi_qspi_write(st->back, AD3552R_REG_ADDR_SCRATCH_PAD,
				     AD3552R_SCRATCH_PAD_TEST_VAL1, SZ_8);
	if (err)
		return err;
	...
	err = ad3552r_axi_qspi_write(st->back,
				     ...
				     AD3552R_SCRATCH_PAD_TEST_VAL2, SZ_8);
	if (err)
		return err;
	...
	err = ad3552r_axi_qspi_write(st->back,
				     ...
				     AD3552R_REF_INIT, SZ_8);
	if (err)
		return err;

	err = ad3552r_axi_qspi_write(st->back,
				     AD3552R_REG_ADDR_TRANSFER_REGISTER,
				     AD3552R_TRANSFER_INIT, SZ_8);
	if (err)
		return err;
