...
/**
 * iio_backend_ext_sync - Enable external synchronization
 * @back: Backend device
 *
 ...
 * (raising and falling) of the interface clock signal.
 *
 * RETURNS:
 * 0 on success, negative error number on failure.
 */
...
/**
 * iio_backend_buffer_enable - Enable DAC data buffering
 * @back: Backend device
 *
 * RETURNS:
 * 0 on success, negative error number on failure.
...
/**
 * iio_backend_set_buffer_disable - Disable DAC data buffering
 * @back: Backend device
 *
 * RETURNS:
 * 0 on success, negative error number on failure.
			     ...
			     u32 reg, u32 *val, size_t val_size)
{
	...
	return iio_backend_op_call(back, bus_reg_read, reg, val, val_size);
}
			      ...
			      u32 reg, u32 val, size_t val_size)
{
	...
	return iio_backend_op_call(back, bus_reg_write, reg, val, val_size);
}
 ...
 * 0 on success, negative error number on failure.
 */
int iio_backend_data_size_set(struct iio_backend *back, size_t size)
{
	if (!size)
		return -EINVAL;
